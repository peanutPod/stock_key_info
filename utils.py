...
def get_stock_info(stock_id):
    ...
    df = pd.DataFrame(
        [{"股票代码": stock_id, "上市时间": ipo_time, "总股本(亿)": total_issue, "总市值(亿)": total_value}]
    )
...
def get_gdhs(stock_id, start_date: str = "19700101", end_date: str = "20500101"):
    ...
    try:
        ...
        # 只保留需要的字段
        df = df[["股东户数统计截止日", "股东户数-本次", "户均持股市值", "股东户数-增减比例", "总股本"]]
...
def get_every_zgb(stock_id_with_market_end, start_year="2020", end_year="2025"):
    ...
    drop_field = ["变更日期"]
    annual_total_shares = annual_total_shares.drop(columns=drop_field, errors="ignore").sort_values(
        by=["年份"], ascending=[False]
    )
    # 设置显示选项，确保以浮点数格式显示
    pd.set_option("display.float_format", "{:.6f}".format)
    print(annual_total_shares)
    # 打印结果
